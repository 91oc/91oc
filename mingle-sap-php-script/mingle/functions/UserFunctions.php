...
function get_membership_status_label( $status ){
	...
	if( $status == '1' ){
    	$membership_status = $sap_common->lang('active');
  	}
  	elseif( $status == '0' ){
  		$membership_status = $sap_common->lang('pending');
  	} 
  	elseif( $status == '2' ){
  		$membership_status = $sap_common->lang('expired'); 
  	} 
  	elseif( $status  == '3'){
  	 	$membership_status = $sap_common->lang('cancelled'); 
  	}
...
function get_payment_status_label( $status ){
	...
	if( $status == '1' ){
    	$recuring_status = $sap_common->lang('install_menu_completed');
  	}
  	elseif( $status == '0' ){
  		$recuring_status = $sap_common->lang('pending');
  	} 
  	elseif( $status == '2' ){
  		$recuring_status = $sap_common->lang('failed');
  	}
  	elseif( $status == '3' ){
  		$recuring_status = $sap_common->lang('refunded');
  	}
  	return $recuring_status;
}
